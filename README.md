...
This method offers the **utmost control and transparency** over the entire build process, making it the unequivocally preferred choice for cryptographers, security researchers, developers, and environments requiring bespoke configurations or in-depth analysis of the source code.
...
|---|---|---|---|
| `mace keygen --label <id>` | Create local identity (`SE P‑256` + `ML‑KEM‑1024`; write `~/.mace/<id>.pub`) | No prompts | Keys are **device-bound** and **non-exportable**, residing within the `Secure Enclave`. This is the **foundational step** for Q-SEAL’s hardware-backed security. |
